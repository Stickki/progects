...
const companyNameToShow = companyName.filter(
...
function searchProductsInput() {
  ...
  const searchArr = items.map(({ id, company, image, price, name }) => {
    ...
    const statement =
      isVisible && price <= maxPriceValue && companyName.includes(company);
...
function showFilterButtons() {
  ...
  productsNavButtons = productsNavList.querySelectorAll(
    ".products__navigation-buttons"
  );
}
  ...
  itemsOnThePage.forEach((allItems) => {
    addedItems.forEach((itemsInCart) => {
      if (itemsInCart.id === parseInt(allItems.getAttribute("data-item-id"))) {
        ...
        const buttonAddedProducts =
          allItems.querySelector(".item-button-added");
  ...
  if (e.target.classList.contains("added__item-remove")) {
    ...
    itemsList.forEach((list) => {
      ...
      listOfItems.forEach((el) => {
        if (
          el.getAttribute("data-item-id") === item.getAttribute("data-item-id")
        ) {
...
function countAddedItemsAmount(e) {
  if (e.target.classList.contains("angle")) {
    ...
    decreaseItems(e);
    itemAmount = parseInt(
      e.target.closest(".added__item").querySelector(".added__item-amount")
        .innerText
    );

    amountArr.push([
      itemAmount,
      e.target.closest(".added__item").getAttribute("data-item-id"),
    ]);
...
function showAmountOfItemInCart() {
  ...
  howManyItems.forEach((item) => {
    cartAllItems += parseInt(
      item.querySelector(".added__item-amount").innerText
    );
    countTotal(
      parseInt(item.querySelector(".added__item-price").innerText.slice(1)) *
        parseInt(item.querySelector(".added__item-amount").innerText)
    );
  });
  countHowManyAdded(cartAllItems);
}
...
function showCartItems({ id, image, name, price }) {
  ...
  return `<div class="added__item" data-item-id="${id}">
                ...
                <div class="added__item-counter">
                    ...
                    <div class="added__item-amount">${
                      amountNum === undefined ? 1 : amountNum
                    }</div>
